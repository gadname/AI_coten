...
const updateImageUrl = (imageKey: keyof typeof imageUrls, newUrl: string) => {
  setImageUrls((prevUrls: typeof imageUrls) => {
    ...
    if (session) {
      ...
      fetch("https://ai-coten.onrender.com/api/v1/our_images/update_urls", { 
        method: 'POST',
        headers: {
...
useEffect(() => {
  const fetchImageUrls = async () => {
    if (session) {
      try {
        const response = await fetch("https://ai-coten.onrender.com/api/v1/our_images", {
          method: 'GET', 
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${(session as CustomSession).accessToken}`,
  ...
  return (
    <div className={styles.root}>
      ...
      <Cpp images={images} onShowModal={showModal} onHideModal={hideModal} isModalVisible={isModalVisible}/>
